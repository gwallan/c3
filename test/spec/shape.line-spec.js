...
describe('c3 chart shape line', function () {
    ...
    describe('point.show option', function () {
        ...
        it('should not show the circle for null', function (done) {
            setTimeout(function () {
                ...
                expect(+target.select('.c3-circle-0').style('opacity')).to.be.equal(1);
                expect(+target.select('.c3-circle-1').style('opacity')).to.be.equal(0);
                expect(+target.select('.c3-circle-2').style('opacity')).to.be.equal(1);
                done();
            }, 500);
        });
        ...
        //     setTimeout(function () {
        //         var target = chart.internal.main.select('.c3-chart-line.c3-target-data1');
        //         expect(+target.select('.c3-circle-0').style('opacity')).to.be.equal(0.5);
        //         expect(+target.select('.c3-circle-1').style('opacity')).to.be.equal(0);
        //         expect(+target.select('.c3-circle-2').style('opacity')).to.be.equal(0.5);
        //         done();
        //     }, 500);
        // });
    ...
    describe('spline.interpolation option', function () {
        ...
        it('should update interpolation function', function() {
            expect(chart.internal.getInterpolate(chart.data()[0])).to.be.equal('cardinal');
        });
